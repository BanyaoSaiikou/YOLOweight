    ...
    while(true){
	    ...
	    for (int i=0; i < msg->markers.size(); i++) {
		    ...
		    // 打印转换后的坐标
                     ...
                     // std::cout << "Marker " << i << " transformed position: "
                     //     << "x=" << bt_v_in.vector.x+0.60279217 << ", y=" << bt_v_in.vector.y+0.08079217 << ", z=" << bt_v_in.vector.z-0.09 << std::endl;

                     std::cout << "Marker " << i << " transformed position: "
                          << "x=" << bt_v_out.vector.x << ", y=" << bt_v_out.vector.y << ", z=" << bt_v_out.vector.z << std::endl;
